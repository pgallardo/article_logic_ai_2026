...
\subsection{Weights for soft constraints}
\label{sec:weights}

We describe a method for assigning weights to soft constraints $Q$ using SBERT retrieval and LLM-based verification. The weight \(w(Q)\in(0,1)\) reflects how strongly the document supports \(Q\) versus contradicts \(Q\). The pipeline uses the following components:
\begin{itemize}
\item \textbf{A bi-encoder} sentence embedding model (e.g., SBERT) for fast retrieval of relevant segments.
\item \textbf{LLM logprob extraction}: We prompt an LLM with retrieved segments and extract logprobs for YES/NO responses regarding whether the text supports $Q$.
\item \textbf{Softmax normalization} to convert logprob differences to confidence scores.
\end{itemize}
...
\begin{itemize}
    \item[1.]
    Given a soft constraint $Q$ expressed as a natural-language hypothesis sentence $h$.
...
Compute the embedding $u = e(h)$ and retrieve the top-$K$ segments by cosine similarity:
\[
E = \text{TopK}_{s_i\in S(T)} \cos(u, v_i),
\]
optionally truncating to a maximum total budget $K_{\text{total}}$ (e.g., 20) by keeping the highest-similarity unique segments.
\item[3.] For each candidate premise segment $p \in E$, construct a verification prompt asking the LLM: ``Based on the following text, is the statement `$h$' supported? Answer YES or NO.'' We request logprobs with \texttt{top\_logprobs=20} to capture the probability distribution over responses.
\item[4.] Extract the logprobs for YES and NO tokens:
\[
(\ell_{\text{yes}}(p), \ell_{\text{no}}(p)) \in \mathbb{R}^2.
\]
Compute the evidence score via softmax:
\[
d(p) := \frac{\exp(\ell_{\text{yes}}(p))}{\exp(\ell_{\text{yes}}(p)) + \exp(\ell_{\text{no}}(p))}.
\]
This yields $d(p) \in (0,1)$ where $d(p) > 0.5$ indicates supporting evidence.
\item[5.] Aggregate evidence across retrieved segments. We take the maximum confidence across segments:
\[
w(Q) := \max_{p \in E} d(p).
\]
\end{itemize}
...
Interpretation:
\begin{itemize}
    \item $w(Q)\approx 1$: strong supporting evidence exists.
    \item $w(Q)\approx 0$: strong contradictory evidence exists.
...
\begin{enumerate}
    \item Retrieve top-$K$ segments $E$ by SBERT similarity to the constraint's natural language form.
    \item For each $p\in E$, prompt the LLM with the segment and constraint, requesting logprobs.
    \item Extract YES/NO logprobs and compute softmax confidence:
    \[
    d(p) = \frac{\exp(\ell_{\text{yes}}(p))}{\exp(\ell_{\text{yes}}(p)) + \exp(\ell_{\text{no}}(p))}.
    \]
    \item Output weight:
...
\begin{itemize}
    ...
    \item SBERT retrieval: $K=20$.
    \item LLM: GPT-4o or similar model supporting logprob extraction.
    \item Logprob setting: \texttt{logprobs=True, top\_logprobs=20}.
\end{itemize}
...
Create a new logified structure from a document:
\begin{verbatim}
python main.py logify --fpath "document.pdf"
python main.py logify --fpath "document.txt" --opath "output.json"
\end{verbatim}

\paragraph{Query mode.}
Ask questions over a logified document:
\begin{verbatim}
python main.py query --fpath "document.pdf" --query "Is X true?"
python main.py query --fpath "doc.pdf" --query "Is X true?" --k 20
\end{verbatim}

In query mode, the system loads or creates the logified structure, translates the query to a formal constraint, and invokes the MaxSAT solver to determine the answer.
