...
Neuro-symbolic methods address this by separating language semantics from logical reasoning: they translate natural language into formal representations and delegate inference to symbolic solvers~\citep{yang2025neurosymbolic}. However, these approaches reconstruct the formalization at each query and assume the full problem fits within the context window. Retrieval-augmented generation (RAG) handles document length but not \emph{scattering}---a non-disclosure clause on page 3 may interact with an exception on page 12---and retrieval cannot guarantee all relevant pieces are surfaced.
