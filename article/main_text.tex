...
\begin{figure}[h!]
    ...
    \caption{Logify system architecture. The document is processed once through a three-stage pipeline: (1) OpenIE triple extraction with coreference resolution, (2) LLM-based conversion to propositions and constraints, and (3) weight assignment via SBERT retrieval and LLM verification. At query time, the user's question is translated to a propositional formula (using SBERT retrieval over the schema), and the MaxSAT solver determines entailment, contradiction, or uncertainty.}
...
We first describe the system architecture, deferring formal definitions to Section~\ref{sec:framework}. The architecture follows a modular design with three components: (1) \texttt{from\_text\_to\_logic}, which logifies text into propositions and constraints; (2) \texttt{logic\_solver}, which performs reasoning via weighted Max-SAT; and (3) \texttt{interface\_with\_user}, which handles natural language interaction. Figure~\ref{fig:architecture} illustrates the overall pipeline.
...
\subsubsection{Weight Output Format}
The weight assignment stage (Stage 3) produces weights for all constraints. The output is stored as a triplet $[\text{prob\_yes\_orig}, \text{prob\_yes\_neg}, \text{confidence}]$ for transparency, where \text{confidence} is the calibrated score used for downstream reasoning. The full algorithm is detailed in Appendix~\ref{sec:weights}.
...
\end{verbatim}
The \texttt{weight} field contains $[\text{prob\_yes\_orig}, \text{prob\_yes\_neg}, \text{confidence}]$ as described above. The full extraction prompt and a worked example are provided in Appendices~\ref{sec:prompt} and \ref{sec:exemplar}.
...
\end{enumerate}
Algebraically, this corresponds to extending the knowledge algebra (Section~\ref{sec:updates}).
