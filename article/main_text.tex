...
While recent work has improved LLM reasoning---chain-of-thought prompting elicits step-by-step inference~\citep{wei2022chain}, self-consistency aggregates over reasoning paths~\citep{wang2023selfconsistency}---neuro-symbolic methods achieve the largest gains by translating natural language into formal representations and delegating reasoning to symbolic solvers~\citep{pan2023logiclm, olausson2023linc}. 
However, these approaches formalize the problem anew at each query and assume the document/input text is relatively short. 
Retrieval-augmented generation (RAG) addresses length but offers no solution when logical constraints are \emph{scattered} across a long document, e.g. a non-disclosure clause on page 3 may interact with an exception on page 12. 
