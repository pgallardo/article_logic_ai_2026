...
Given a long document—a medical guideline, a legal contract, a technical
specification—users need to ask questions, check consistency, and understand
what follows from what. Large language models (LLMs) offer fluent interfaces
but struggle with such reasoning: they hallucinate facts \cite{ji2023hallucination},
miss information in long contexts \cite{liu2024lost}, and cannot guarantee
faithful inference \cite{pan2023logiclm}.

A common mitigation is retrieval-augmented generation (RAG), where relevant
passages are retrieved and fed to an LLM at query time. But when logical
constraints are \emph{scattered} across a long document, retrieval may miss
critical pieces, and the LLM must reason over incomplete or inconsistent
context. The result is unreliable answers with no formal guarantees.

We propose a different paradigm: \emph{logify} the document once, then reason
symbolically over the resulting structure. Our pipeline extracts atomic
propositions (statements that can be true or false) and two kinds of constraints:
\begin{itemize}
    ...
    (e.g., ``typically,'' ``may'')
\end{itemize}
The hard constraints define consistent \emph{readings} of the document; the
soft constraints rank readings by plausibility. This structure supports faithful
reasoning—entailment, inconsistency detection, quantification of uncertainty—via
a weighted Max-SAT solver. The LLM serves only as a natural-language interface;
all reasoning is delegated to the solver. This separation ensures \emph{conditional}
faithfulness: given a correct logification, the answers are provably correct.
The extraction step remains LLM-based and thus fallible, but errors are localized
to extraction rather than reasoning.

On ContractNLI, Logify outperforms Logic-LM by 12.5\%, with the largest gains on
long documents where constraints are scattered. On ProofWriter, it maintains
consistent accuracy even at reasoning depth 5, where chain-of-thought degrades
sharply.
